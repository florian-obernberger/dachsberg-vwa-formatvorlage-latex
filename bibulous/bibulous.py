...
__editors__ = ['Florian Obernberger']
__license__ = 'MIT/X11 License'
__contact__ = 'Nathan Hagen <and.the.light.shattered@gmail.com>'
__version__ = '1.3'
__all__ = [
    'sentence_case', 'stringsplit', 'namefield_to_namelist', 'namestr_to_namedict',
    'initialize_name', 'get_delim_levels', 'show_levels_debug', 'get_quote_levels', 'splitat',
    'multisplit', 'enwrap_nested_string', 'enwrap_nested_quotes', 'purify_string', 'latex_to_utf8',
...
class Bibdata(object):
    ...
    def __init__(self,
        ...
        self.filedict = {
        }  ## the dictionary containing all of the files associated with the bibliography
        self.citedict = {}  ## the dictionary containing the original data from the AUX file
        self.citelist = [
        ]  ## the list of citation keys, given in the order determined by "self.sortlist"
        ...
        self.implicitly_indexed_vars = ['authorname',
                                        'editorname']  ## which templates have implicit indexing
        self.implicit_loop_pairs = {
        }  ## the dictionary pairing name variable with namelist (i.e. "authorname" with "authorlist")
        ...
        self.auxfile_list = [
        ]  ## a list of *.aux files, for use when citations are inside nested files
        ...
        if (uselocale == None):
            ...
            self.locale = locale.setlocale(locale.LC_ALL,
                                           uselocale)  ## set the locale as requested
        ...
        self.index_pattern = re.compile(r'(<' + pat + r'\.\d+\.' + pat + r'>)|(<' + pat +
                                        r'\.\d+>)|(<' + pat + r'\.(nN)\.' + pat + r'>)|(' + pat +
                                        r'\.(nN)>)')
        self.implicit_index_pattern = re.compile(r'(<' + pat + r'\.n\.' + pat + r'>)|(<' + pat +
                                                 r'\.n>)')
        ...
        for i in range(1, 13):
            if not (platform.system() == 'Windows'):
                ...
                self.monthabbrevs[str(i)] = locale.nl_langinfo(locale.__dict__['ABMON_' +
                                                                               str(i)]).title()
            ...
            if (platform.system() == 'Windows'):
                ...
                self.monthnames[str(i)] = locale.nl_langinfo(locale.__dict__['MON_' +
                                                                             str(i)]).title()
        ...
        if self.filedict['bib']:
            if self.citedict and self.options['use_citeextract'] and os.path.exists(
                    self.filedict['extract']):
    ...
    def parse_bibfile(self, filename):
        ...
        for line in filehandle:
            ...
            if line.startswith('}'):
                ...
                if (line[1:].strip() != ''):
                    bib_warning(
                        'Warning 001a: line#' + str(self.i) + ' of "' + self.filename +
                        '" has data outside'
                        ' of an entry {...} block. Skipping all contents until the next entry ...',
                        self.disable)
                continue
            ...
            if line.startswith('@'):
                ...
                if (brace_idx == -1):
                    ...
                    entry_brace_level = 0
                    continue
                entrytype = line[1:brace_idx].lower().strip(
        ...
        if (entrytype == 'comment'):
            ...
            for fdkey in fd:
                if (fdkey in self.abbrevs):
                    bib_warning(
                        'Warning 032a: line#' + str(self.i) + ' of "' + self.filename +
                        ': the abbreviation "' + fdkey + '" = "' + self.abbrevs[fdkey] +
                        '" is being '
            ...
            if (entrykey in self.bibdata):
                bib_warning(
                    'Warning 032b: line#' + str(self.i) + ' of "' + self.filename +
            ...
            if not self.parse_only_entrykeys:
                ...
                if preexists:
                    bib_warning(
                        'Warning 032c: line#' + str(self.i) + ' of "' + self.filename +
                        ': the entry "' + entrykey +
                        '" is being overwritten with a new definition', self.disable)
    ...
    def parse_bibfield(self, entrystr, entrykey=''):
        ...
        while entrystr:
            ...
            if (fieldkey in fd):
                bib_warning(
                    'Warning 033: line#' + str(self.i) + ' of "' + self.filename + ': the "' +
                    fieldkey + '" field of entry "' + entrykey + '" is duplicated', self.disable)
            ...
            while fieldstr:
                ...
                if (firstchar == ','):
                    ...
                    if not re.search(self.abbrevkey_pattern, fieldstr):
                        ...
                        (fieldstr, resultstr,
                         end_of_field) = self.replace_abbrevs_with_full(fieldstr, resultstr)
    ...
    def parse_auxfile(self, filename, recursive_call=False, debug=False):
        ...
        if not self.keylist:
            bib_warning('Warning 007: no citations found in AUX file "' + filename + '"',
                        self.disable)
        else:
    ...
    def parse_bstfile(self, filename):
        ...
        for i, line in enumerate(filehandle):
            ...
            if ('EXECUTE {' in line) or ('EXECUTE{' in line) or ('FUNCTION {' in line):
                raise ImportError('The style template file "' + filename +
                                  '" appears to be BibTeX format, not '
                                  'Bibulous. Aborting...')
                ...
                if (section == 'TEMPLATES'):
                    ...
                    if re.search('\.n\.', value) or re.search('\.n>', value):
                        ...
                        if (varname not in self.implicit_loop_pairs):
                            ...
                            self.implicit_loop_pairs[varname] = value[:new_start].strip(
                                '[<').strip()
        ...
        try:
            ...
            for c in self.citelist:
                ...
                if (s != ''):
                    ...
                    entries.append(s)
                    # filehandle.write(s + '\n\n')
            for e in sorted(entries, key=lambda x: x.split('\n')[1].replace('{', '')):
                filehandle.write(e + '\n\n')
    ...
    def format_bibitem(self, citekey, debug=False):
        ...
        if (entrytype in self.bstdict):
            ...
            if (str(self.bibdata[c]['citelabel']) == 'None'):
                ...
                itemstr = r'\bibitem[' + self.bibdata[c][
                    'citelabel'] + ']{' + c + '}\n' + self.bibdata[c]['errormsg']
            return (itemstr)
        else:
    ...
    def insert_crossref_data(self, entrykey, fieldname=None):
        ...
        if ('title' in crossref_keys) and ('booktitle' not in self.bibdata[entrykey]):
            self.bibdata[entrykey]['booktitle'] = self.bibdata[self.bibdata[entrykey]
                                                               ['crossref']]['title']
    ...
    def write_citeextract(self, outputfile, write_abbrevs=False):
        ...
        for key in self.citedict:
            ...
            if ('crossref' in self.bibdata[key]) and (self.bibdata[key]['crossref']
                                                      in self.bibdata):
    ...
    def write_authorextract(self, searchname, outputfile=None, write_abbrevs=False):
        ...
        for k in self.bibdata:
            ...
            if ('author' in self.bibdata[k]):
                name_list_of_dicts = namefield_to_namelist(self.bibdata[k]['author'],
                                                           key=k,
                                                           sep=sep,
                                                           disable=self.disable)
            if ('editor' in self.bibdata[k]):
                namelist = namefield_to_namelist(self.bibdata[k]['editor'],
                                                 key=k,
                                                 sep=sep,
                                                 disable=self.disable)
                ...
                for namekey in searchname:
                    if (namekey in name):
                        if name_is_initialized[namekey]:
                            thisname = initialize_name(
                                name[namekey], options={'period_after_initial': True}) + '.'
                        else:
    ...
    def insert_specials(self, entrykey):
        ...
        for key in self.specials_list:
            ...
            template_has_implicit_index = True if re.search(self.implicit_index_pattern,
                                                            templatestr) else False
        ...
        if not has_implicit_loop and not has_index:
            ...
            for v in indexed_vars:
                ...
                if (varname in self.specials):
                    ...
                    templatestr = re.sub(r'(?<=\.)n(?=\.)|(?<=\.)n(?=>)', elem['index'],
                                         templatestr)
            return (templatestr)
    ...
    def template_substitution(self, templatestr, entrykey, templatekey=None):
        ...
        if is_nested:
            templatestr = self.remove_nested_template_options_brackets(
                templatestr, bibentry, variables)
        else:
        ...
        for var in variables:
            ...
            if varname.startswith('title') and ('title' in bibentry):
                ...
                templatestr = templatestr.replace(var,
                                                  create_citation_alpha(bibentry, self.options))
                continue
    ...
    def insert_title_into_template(self, title_var, templatestr, bibentry):
        ...
        if title.endswith(('?', '!')):
            ...
            if (templatestr[idx + len(title_var)] in (',', '.', '!', '?', ';', ':')):
                templatestr = templatestr[:idx + len(title_var)] + templatestr[idx + 1 +
                                                                               len(title_var):]
    ...
    def remove_nested_template_options_brackets(self, templatestr, entry, variables):
        ...
        while ('[' in templatestr):
            ...
            for block in blocks:
                ...
                newblock = self.remove_nested_template_options_brackets(
                    block, entry, block_variables)
    ...
    def get_variable(self, bibentry, variable, options={}):
        ...
        result = self.get_indexed_variable(bibentry[fieldname],
                                           indexer,
                                           bibentry['entrykey'],
                                           options=options)
        ...
        if index_elements[0].isdigit():
            if not isinstance(field, (tuple, list)):
                ...
                if (nelements == 1):
                    ...
                    return (self.get_indexed_variable(newfield,
                                                      newindexer,
                                                      entrykey,
                                                      options=options))
            ...
            if indexer.startswith('.initial()'):
                ...
                if (nelements == 1) or (newindexer == ''):
                    ...
                    return (self.get_indexed_variable(newfield,
                                                      ...
                                                      entrykey,
                                                      options=options))
            elif indexer.startswith('.current_year()'):
                return (str(datetime.now().year))
            elif indexer.startswith('.first_word()'):
                ...
                if (nelements == 1) or (newindexer == ''):
                    ...
                    return (self.get_indexed_variable(newfield,
                                                      ...
                                                      entrykey,
                                                      options=options))
            elif indexer.startswith('.if_not_one()'):
                if field in ('1', 1):
                    newfield = ''
                else:
                    newfield = ' ' + field
                ...
                sep = self.options['name_separator']
                newfield = namefield_to_namelist(field,
                                                 key=entrykey,
                                                 sep=sep,
                                                 disable=self.disable)
                newindexer = indexer[14:]
                ...
                if (field in self.uniquify_vars[options['varname']]):
                    ...
                    while True:
                        if (i < 27):
                            ...
                            newfield = field + chr(q + 96) + chr(
                                q + 96)  ## double up if a single append doesn't work
                        elif (i < 78):
                            newfield = field + chr(q + 96) + chr(q + 96) + chr(
                                q + 96)  ## triple up if necessary
                        newfield += str(q)
                        q += 1
                        if (newfield not in self.uniquify_vars[options['varname']]):
                            break
                ...
                if (nelements == 1) or (newindexer == ''):
                    ...
                    return (self.get_indexed_variable(newfield,
                                                      ...
                                                      entrykey,
                                                      options=options))
            elif indexer.startswith('.if_multiple('):
                match = re.search(r'.if_multiple(\(.*\))', indexer)
                end_idx = match.end(0)
                result = match.group(0)[13:-1]  ## remove function name and parentheses
                ...
                (variable_to_eval, test_length, return_if_equal,
                 return_if_unequal) = result.split(',')
...
def namefield_to_namelist(namefield, key=None, sep='and', disable=None):
    ...
    if re.search(r'\s' + sep + ',\s', namefield):
        bib_warning(
            'Warning 017a: The name string in entry "' + key + '" has " ' + sep +
            ', ", which is likely a'
            ' typo. Continuing on anyway ...', disable)
    if re.search(r', ' + sep, namefield):
        bib_warning(
            'Warning 017b: The name string in entry "' + key + '" has ", ' + sep +
            '", which is likely a'
            ' typo. Continuing on anyway ...', disable)
    if re.search(r'\s' + sep + '\s+' + sep + '\s', namefield):
        bib_warning(
            'Warning 017c: The name string in entry "' + key + '" has two "' + sep +
    ...
    namelist = sorted(namelist, key=lambda x: x["last"])
    return (namelist)
...
def initialize_name(name, options=None, debug=False):
    ...
    nametokens = [
        x for x in nametokens
        if x and not (x.startswith('(') or x.endswith(')') or x.startswith('[') or x.endswith(']'))
    ]
...
def get_quote_levels(s, disable=None, debug=False):
    ...
    for j, ch in enumerate(s):
        if (ch == '`') and (s[j - 1] != '\\'):
            ...
            elif (s[j - 1] != '`') and (s[j - 1].isspace() or (s[j - 1] in '()[]{}":;,<>') or
                                        (j == 0)):
...
## ===================================
def enwrap_nested_string(s,
                         delims=('{', '}'),
                         odd_operator=r'\textbf',
                         even_operator=r'\textrm',
                         disable=None):
    '''
    ...
    if (oplevels[-1] > 0):
        bib_warning(
            'Warning 019: found mismatched "{","}" brace pairs in the input string. Ignoring the problem and'
...
def enwrap_nested_quotes(s, disable=None, debug=False):
    ...
    if ("```" in s) or ("'''" in s):
        bib_warning(
            'Warning 020: the input string ["' + s +
            '"] contains multiple unseparated quote characters.'
...
def namestr_to_namedict(namestr, disable=None):
    ...
    if (',' in namestr):
        if (namestr.strip() == ','):
            bib_warning(
                'Warning 038: A name in the bibliography file contains only a lone comma, and so is a typo. Skipping ...',
                disable)
            return ({})
        ...
        for match in re.finditer(',', namestr):
            ...
            if (len(namestr) > i) and not (namestr[i] == ',') and not namestr[i + 1].isspace():
                bib_warning(
                    'Warning 037: A comma appears within the name string "' + namestr +
                    '" and has no whitespace following it, and so is likely a typo. Ignoring ...',
                    disable)
...
def get_edition_ordinal(edition_field, disable=None):
    ...
    if (edition_field == '0'):
        bib_warning('Warning 024: an edition number of "0" is invalid. Cannot create ordinal.',
                    disable)
        return (edition_field)
...
def parse_pagerange(pages_str, citekey=None, disable=None):
    ...
    if startpage.isdigit() and endpage and endpage.isdigit():
        if int(endpage) < int(startpage):
            if (citekey != None):
                bib_warning(
                    'Warning 025a: the "pages" field in entry "' + citekey +
                    '" has a malformed page range '
                    '(endpage < startpage). Ignoring ...', disable)
            else:
                bib_warning(
                    'Warning 025b: the "pages" field "' + pages_str +
                    '" is malformed, since endpage < '
...
def get_implicit_loop_data(templatestr):
    ...
    if re.search(r'\{.*?\}', rhs_glue):
        ...
        last_glue = rhs_glue[:glue_start] + rhs_glue[glue_start + 1:glue_end] + rhs_glue[glue_end +
                                                                                         1:]
    else:
